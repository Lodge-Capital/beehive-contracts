...
contract BeehiveEscrow is IERC721, IERC721Metadata, IVotes {
  enum DepositType {
    DEPOSIT_FOR_TYPE,
    CREATE_LOCK_TYPE,
  ...
  /// @notice RewardDistributor
  address public beehive;

  /// @notice Contract constructor
  /// @param token_addr `DUES` token address
  ...
  string public constant name = "BEEHIVE NFT";
  ...
  function _checkpoint(
    ...
    {
      uint t_i = (last_checkpoint / _WEEK) * _WEEK;
  ...
  function _supply_at(Point memory point, uint t) internal view returns (uint) {
    Point memory last_point = point;
    uint t_i = (last_point.ts / _WEEK) * _WEEK;
    for (uint i = 0; i < 255; ++i) {
      t_i += _WEEK;
      int128 d_slope = 0;
      if (t_i > t) {
  ...
  function delegateBySig(
    ...
    require(
      signatory != address(0),
      "BeehiveEscrow::delegateBySig: invalid signature"
    );
    require(
      nonce == nonces[signatory]++,
      "BeehiveEscrow::delegateBySig: invalid nonce"
    );
    require(
      block.timestamp <= expiry,
      "BeehiveEscrow::delegateBySig: signature expired"
    );
