...
import "contracts/interfaces/IBeehiveEscrow.sol";
...
contract BeehiveDistributor is IRewardsDistributor {
  ...
  constructor(address _voting_escrow) {
    ...
    last_token_time = _t;
    time_cursor = _t;
    address _token = IBeehiveEscrow(_voting_escrow).token();
    token = _token;
  ...
  function _find_timestamp_epoch(
    ...
    uint _min = 0;
    uint _max = IBeehiveEscrow(ve).epoch();
    for (uint i = 0; i < 128; i++) {
      if (_min >= _max) break;
      uint _mid = (_min + _max + 2) / 2;
      IBeehiveEscrow.Point memory pt = IBeehiveEscrow(ve).point_history(_mid);
      if (pt.ts <= _timestamp) {
        _min = _mid;
      } else {
        _max = _mid - 1;
  ...
  function _find_timestamp_user_epoch(
    ...
    for (uint i = 0; i < 128; i++) {
      if (_min >= _max) break;
      uint _mid = (_min + _max + 2) / 2;
      IBeehiveEscrow.Point memory pt = IBeehiveEscrow(ve).user_point_history(
        tokenId,
        _mid
      );
      if (pt.ts <= _timestamp) {
  ...
  function ve_for_at(
    ...
    uint max_user_epoch = IBeehiveEscrow(ve).user_point_epoch(_tokenId);
  ...
  function _checkpoint_total_supply() internal {
    ...
    for (uint i = 0; i < 20; i++) {
      if (t > rounded_timestamp) {
        ...
        IBeehiveEscrow.Point memory pt = IBeehiveEscrow(ve).point_history(
          epoch
        );
        int128 dt = 0;
        if (t > pt.ts) {
  ...
  function _claim(
    ...
    IBeehiveEscrow.Point memory user_point = IBeehiveEscrow(ve)
    ...
    IBeehiveEscrow.Point memory old_user_point;
    ...
    for (uint i = 0; i < 50; i++) {
      ...
      if (week_cursor >= user_point.ts && user_epoch <= max_user_epoch) {
        ...
        if (user_epoch > max_user_epoch) {
          user_point = IBeehiveEscrow.Point(0, 0, 0, 0);
        } else {
          user_point = IBeehiveEscrow(ve).user_point_history(
            _tokenId,
            user_epoch
          );
        }
